...
\begin{algorithm}
  ...
  \While{time\_interval in time\_intervals}{
    ...
    Build system\_dependency\_graph using service\_dependencies\;
    Extract graph\_metrics from system\_dependency\_graph\;
...
\begin{algorithm}
  \KwData{Trace files/Trace data.}
  \KwResult{CSV with unique workflow types, their corresponding count and times.}
...
\begin{algorithm}
  \KwData{Trace files/Trace data.}
  \KwResult{CSV file reporting span structure analysis.}
...
\begin{algorithm}
  \KwData{Trace files/Trace data.}
  \KwResult{CSV file for each service reporting the coverability analysis.}
  Read start\_time and end\_time from configuration\;
  Get services from Zipkin\;
  \While{service in services}{
    ...
    Write trace\_coverability to CSV file\;
  }
